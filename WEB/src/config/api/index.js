...
class ApiCounstructor {
  ...
  //
  apiBuilder ({ apis = {}, devBaseUrl, prodBaseUrl, isDevMode = false, sep = '/' }) {
    Object.keys(apis).forEach(namespace => {
      this.buildApiWidthNamespace({ namespace, apis: apis[namespace], isDevMode, devBaseUrl, prodBaseUrl, sep })
    })
  }

  buildApiWidthNamespace ({ namespace, apis, devBaseUrl, prodBaseUrl, isDevMode, sep }) {
    apis.forEach(api => {
      const { name, method, path, mockPath, params, desc } = api
      const apiNamespace = `${namespace}${sep}${name}`
      const url = isDevMode ? `${devBaseUrl}${mockPath}?random=${Math.random()}` : `${prodBaseUrl}${path}`
      // const url = `${prodBaseUrl}${path}?random=${Math.random()}`
...
export const api = new ApiCounstructor({ apis: apis, ...API_DEFAULT_CONFIG }).api
